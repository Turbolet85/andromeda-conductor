...
| Terminal output rendering | owo-colors 4 + indicatif 0.17 + comfy-table 7 + inquire 0.9 | `conductor-cli` presentation layer (Epoch 8): tty-gated status-line color (never color-alone), run/suite progress spinner, results + 60-P-ID coverage tables, isatty-gated interactive operator-pause prompts (inquire confirm; headless never blocks) |
...
- `contracts/pulse-capabilities.toml` — the versioned SUT capability manifest: the accepted Pulse capability set (`sut_version` · `captured_at` · `capabilities[]`), the single source of which P-IDs a scenario may name. Runtime-read and bounds-checked at load; resolved from a fixed `CapabilityManifest::default_path()` through `resolve_under` with deliberately NO `CONDUCTOR_*` override handle (unlike `CONDUCTOR_CONTRACT_MANIFEST`). Re-aiming Conductor at a newer Pulse release is an edit to this file, not a code change.
