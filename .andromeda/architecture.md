...
| Desktop frontend (webview) | React 19.x + Vite 8.0.16 + Tailwind v4.1 (Oxide via `@tailwindcss/vite`) + Fontsource WOFF2; npm | Optional GUI's SPA under `crates/conductor-tauri/ui/` (npm, NOT a Cargo member); design tokens on `:root` (Tailwind v4 `@theme` tree-shakes non-namespace tokens), self-hosted fonts (no CDN); `npm audit` gate + committed `package-lock.json` |
