...
- **[Validation Library] serde 1.0.x + garde 0.22.1:** garde (the modern rewrite of `validator`) expresses scenario bounds declaratively next to the serde structs — `range` rules for simple bounds (error fraction ∈ [0,1], non-negative durations, sane ramp factors) and `#[garde(custom)]` field validators for the rest; cross-field invariants spanning distinct fields (p50≤p95≤p99 ordering, severity-mix sums) use garde's `Context` pattern, since garde 0.22.1's `custom` is field-level only (no container-level custom) — with no async or web-framework coupling. **Pinned 0.22.1**, not the originally-chosen 0.23.0: its `garde_derive 0.23.0` proved absent from the registry, so 0.23.0 + the `derive` feature is unbuildable. validator (predecessor) and serde_valid (JSON-Schema niche) were rejected.
...
├─ contracts/                 # pinned MCP contract manifest + the SUT capability manifest
