...
> - `refs/pulse-mcp-contract.md` — the MCP read-back seam pinned from Pulse source (`crates/mcp-server/`):
>   stdio child-process `andromeda-pulse-mcp`, protocol `2024-11-05` (not the rmcp default), and the
>   load-bearing data-dir rule — the sidecar must be spawned with `ANDROMEDA_PULSE_DATA_DIR` equal to the
>   live Pulse's or read-back is silently empty. Required reading for the verification layer.
...
4. **Verification layer**:
   ...
   - **MCP read-back client** over Pulse's tested tool surface — `query_incident_list` ·
     ...
     with the non-default `mcp-server` cargo feature AND runtime env `ANDROMEDA_PULSE_MCP_ENABLED` AND
     the sidecar spawned with `ANDROMEDA_PULSE_DATA_DIR` equal to the live Pulse's data-dir (the sidecar
     reads `{data_dir}/corpus/corpus.db` and filters incidents by the data-dir-derived workspace —
     mismatch ⇒ read-back silently empty; preflight must canary-round-trip a known incident before any
     scenario trusts read-back — see `refs/pulse-mcp-contract.md`);
