...
**Tooling context:** Framework React 19.x (Vite SPA; Preact 10.x size fallback), CSS Tailwind v4.1 (Oxide static stylesheet; design tokens declared on `:root`, never `@theme`), Components shadcn/ui (Radix UI Primitives 1.x, locally vendored) + Lucide React icons. Tauri 2 (≥ 2.10.3) frameless window (`decorations: false`).
...
| │  1 of 1 unconfirmed                                   │ |     unticked roll-up (role=status)
...
|                                                          |     text-secondary), printed ONCE per run, outside the lamp column;
|                                                          |     ABSENT entirely when the run stayed in-envelope
...
- **Capability** — the classification's title (Body role) above its category (Label role, `text-tertiary`).
- **Mode** (Data role) — the definition-of-done classification, rendered as text in exactly one of four values: `auto` · `drive+observe` · `static-only` · `not-Conductor's` (the last = outside Conductor's remit by standing non-goal, verified by the SUT's own suites). Never color-only; the header-strip tally counts all four, summing to the row count. The cli `conductor coverage` table mirrors the same four values as text. **The out-of-scope value carries a recessive treatment** — `--status-residual` (webview, via a Mode-cell modifier) ↔ ANSI 246 (cli) ↔ emphasis (Markdown, which has no color channel) — adapted per surface, never forked. It is deliberately NOT `--status-fail` or `--count-blocked`: out-of-scope states REMIT, not an outcome, so reusing ANY lamp-set token would read as a verdict/report-state. (Until 2026-09-01 this was justified by `--count-blocked` sharing a hex with `--text-muted`; that identity ended when `--text-muted` moved for contrast, and the ruling never depended on it — it rests on the lamp-token SET, not on a hex coincidence.) The mode label itself always renders, so the classification survives `NO_COLOR` / piping / monochrome, and the tint never becomes a seventh lamp state (the lamp column is untouched).
- **Status** — the 6-state verdict/report-state lamp, LAST in the row: filled dot for a measured verdict (`Pass`/`CalibrationRegion`/`Fail`) · hollow ring `Blocked` (never measured) · neutral checkbox `ManualCheck` (awaiting operator) · muted dashed dot `KnownResidual` (pre-accepted gap), `radius-full` at the icon size grid, always paired with its status text. A capability **no run record names** renders "Not yet run" prose (Body, `text-tertiary`) in this cell instead — an ABSENCE state, distinguished from a failure by its TEXT, never by tint alone. Where several of a run's records name one P-ID the row shows the WORST of their lamps (`Fail > Blocked > Hold > Manual > Residual > Pass`), so a sibling scenario's block is never hidden behind another's pass.
...
**States:** selected row → `border-emphasis` left edge; hover → `color-raised-1` lift `motion-micro`. **Empty** → the console's empty-CATALOG prose, the shipped `No scenarios found.` (Body, `text-tertiary`; owned by `App.tsx`, not this component — measured 2026-09-02, SR row E0-01), never a gray placeholder. **Distinct from the picker's FILTER-MISS state**, which renders its own shipped `No scenarios match.` from a persistently-mounted announced region (SR row S0-16, measured 2026-09-04) — two states, two strings, two rows; neither is the other's. **A THIRD App.tsx-owned state sits beside them: the catalog LOAD-ERROR render** (sanitized error prose, `status-fail`, paired text — never tint alone). Its region was already mounted-empty-at-first-paint; what changed 2026-09-07 is the SITING — the VISIBLE copy now sits OUTSIDE the announced `role="alert"` region and the region carries a visually-hidden re-assertion alone, inserted once on the first `focusin`, so the message is announced exactly ONCE (keeping both inside made NVDA read the region whole and speak it twice — as measured at SR row R0-01, `2026-09-07-sr-findings-fixed`). The three sibling error regions (action / coverage / report) keep the empty-mount shape, because they fire at arbitrary times rather than before the screen reader binds. The header strip carries the matrix counts (Data, `color-id-cyan`) and — while held — echoes the frozen step-index in `count-hold` (signature reinforcement #3). Every lamp is paired with its status text (`Pass` / `CalibrationRegion` / `Fail` / `Blocked`) so state is never color-only, and a verdict change is announced (a11y derives the live-region attribute).
...
- **Run-level load-envelope banner** — a single qualifier row ABOVE the per-P-ID verdict lines and OUTSIDE the lamp column, carrying the envelope label plus its cause: recessive `status-residual` label over `color-raised-1`, `1px border-subtle`, `radius-md`, `space-sm`/`space-md` padding, cause in `text-secondary`, `motion-micro`/`ease-quiet` color transition. The label text ALWAYS renders, so the tint is never the only signal. It is neither a seventh lamp nor a sixth `ReportState` (the lamp set stays closed at six — the `not-conductors` Mode-cell precedent in §Primary content block 1), and it is **omitted entirely** when the run stayed in-envelope. This is the webview leg of the qualifier the cli prints as `[ENVIRONMENT-SUSPECT]` and the Markdown report banners — adapted per surface, never forked.
...
- **`conductor preflight [--json]`** — the MCP readiness gate (no longer boot's FIRST act — the probe below leads, and short-circuits past this gate only when a subject is genuinely unmet): emits the `ReadyState` JSON (or a `[PASS]`/`[BLOCKED]` line) and exits 0 iff `ready:true`, non-zero on a Blocked precondition. **`agent-run boot` REACHES it** — the leading probe runs first and short-circuits only on a genuinely unmet subject; on exit 0 boot proceeds here and emits the `ReadyState` JSON, measured in the full shipped shell SET (`scripts/agent-run.sh` 2026-09-04T17:07:17→17:08:04Z · `scripts/agent-run.ps1` 17:11:16→17:12:03Z, both exit 0 with `ready: true`, zero `skipped preflight` lines, neither script edited; `conductor-0.2.0/chunks/2026-09-04-preconditions-probe-reads-path-handles-by-presence/report.md`). Invoking this verb directly remains a valid entry point, not a forced workaround.
- **`conductor preconditions [--for <SCENARIO>] [--json]`** — the non-mutating live-Pulse probe run BEFORE a leg is scheduled: `:4317` reachability as a connect-and-drop client, `andromeda-pulse-mcp` resolved over the inherited `PATH` without spawning, and the three `ANDROMEDA_PULSE_*` handles read for an affirmative declaration under the deterministic posture — the flagless default. `--for` (2026-09-22) takes a scenario NAME — a `P-NNN`-shaped value is refused at argument parsing (a clap usage error, exit 2), because a P-ID can name several scenarios — and grades the handles under that scenario's declared `l4_posture`: under `real-model` the L4 handle is read for ABSENCE instead, its unmet line reading `declared in this environment, but the real-model posture requires it absent or falsy`. Exits 0 iff every subject is satisfied, non-zero otherwise (the documented `preflight` go/no-go exception, not the scenario-verdict rule) — and **exit 0 IS reachable**: each handle is graded by its own KIND, the PATH-valued member by presence-after-trim and the flag-valued members by an affirmative `"true"`/`"1"`, so `handles-declared` is satisfiable (as measured at `conductor-0.2.0/chunks/2026-09-04-preconditions-probe-reads-path-handles-by-presence/report.md`, 2026-09-04T17:07Z) — and under a `--for` real-model scenario the L4 flag is unmet when EITHER side's truthiness rule declares it (Conductor's `true`/`1` ∪ Pulse's `1`/`true`/`yes`, trimmed and ASCII-lowercased), every other handle graded as before; exit 0 was measured reachable under that posture too (2026-09-23). Both arms of the `[PRECONDITION]` caption are shipped, measured behaviour: ANSI 246, one line per unmet subject naming its statement and candidate causes when any subject is unmet, and the single satisfied line (`[PRECONDITION] every live-Pulse precondition is satisfied`) when none is — so a reader keying on the bracket label must not read it as failure-only. Mints no `Verdict`, no `ReportState` and no per-P-ID row. It fires no canary, so it primes none of the SUT state a preflight would.
...
- **`conductor cleanup <run_id>`** — the teardown verb (2026-09-06): removes one stored run's rows from all three `runs.db` tables in a single transaction (`RunsDb::delete_run`), printing a plain confirmation line carrying the removed row count with the run id in the ID-cyan mapping. Idempotent — a second call removes nothing and is not an error. Mints NO bracket label, no lamp, no `ReportState` and no new token or ANSI entry; it never prompts, so the headless path stays unblocked. The `run_id` binds as a SQL `?1` parameter only and is never joined into a path (unlike `report`). This is the mechanism `scripts/agent-run.{sh,ps1} cleanup` now invokes behind its existing run-id guard — the shells issue no SQL of their own.
- **`scripts/agent-run.{sh,ps1}`** (both shells ship at identical semantics — test-plan §3; a property of the SCRIPTS together with the invoking environment, since a caller preempting native-command exits diverges the `.ps1` leg with zero script delta — measured 2026-09-07) — the headless, agent-driven source-of-truth path (the 5-command `boot`/`run`/`status`/`cleanup`/`logs` harness; `run` takes `--unit`/`--integration`/`--e2e`/`--live` stage flags, default = the full bundled gate). **`--live` is the operator-gated live-Pulse suite** (2026-09-06) — a sibling stage flag, never a sixth command, reachable from no default or CI path. It leads with the non-priming `conductor preconditions` probe: an unmet subject REFUSES the suite at exit 1, printing one host-path-free `[PRECONDITION]` line per unsatisfied subject with its candidate causes, and no leg fires. A third-token selector (2026-09-22) picks the other shape: **`--live real-model`** runs the ONE operator-gated real-model leg (`real-model-interpretation`) instead of the suite, leading with `conductor preconditions --for real-model-interpretation` and refusing the same way; an unknown selector prints `usage: <script> run --live [real-model]` and exits 2 before any probe, in both shells — a selector of `--live`, neither a sixth command nor a new stage flag. That refusal is distinct from a HOST-TOOL handle's absence (`CONDUCTOR_MSEDGEDRIVER` / `CONDUCTOR_NVDA`), which SKIPS its own leg at exit 0 with a fetch recipe — an incomplete live-Pulse environment is a refusal, a missing host tool is a skip. That host-tool skip is the LAX arm: under an affirmative `CONDUCTOR_A11Y_STRICT` (the CI posture, 2026-09-07) the same unresolved handle exits non-zero instead, so the two-class contrast holds for the default arm and CI adds a third outcome rather than replacing either. `--unit`/`--integration` and the bundled default are same-engine cli legs, TTY-gated so artifacts stay clean and the operator-pause is **never** blocked on an interactive prompt. `--e2e` is the cross-surface stage: it builds the release Tauri bundle and drives the desktop-webview console through the tauri-driver webview leg, gated on a Conductor-owned driver handle whose absence skips the stage at exit 0 with a host-path-free `error:`/`hint:` precondition — never a hard failure on the DEFAULT arm, and a non-zero exit when `CONDUCTOR_A11Y_STRICT` is affirmatively declared (2026-09-07). It also asserts the runner's PRINTED verdict rather than its exit code — the `Spec Files:` failed count, the per-spec skip markers against the expected-skip set, and the `[webview2 … windows]` driven-session banner — because the exit code alone cannot tell a full pass from a total skip.
...
Those six are the complete set of **per-P-ID verdict / report-state** bracket labels, and the lamp set is closed at six. Each surface additionally carries a **run-level, non-lamp** qualifier SET, printed outside the lamp column — on cli that is `[ENVIRONMENT-SUSPECT]` (load-envelope, once per run) and `[PRECONDITION]` (the `conductor preconditions` probe — one line per unmet subject, or the single satisfied line when every subject is met; both arms are reachable, so it is not a failure-only label) (the `not-conductors` Mode-cell precedent in §Primary content block 1) — a SET adapted per surface, never forked: cli stdout's `[ENVIRONMENT-SUSPECT]` bracket label in ANSI 246, the Markdown report's banner, and the desktop-webview run-report banner binding the same pair by name as `var(--status-residual)` (§Primary content block 2). It qualifies the run rather than reporting a check, so it is neither a seventh lamp nor a sixth `ReportState`; a parser keying on state labels should read the per-P-ID set plus that non-lamp qualifier SET.
