...
- **`conductor preflight [--json]`** — the MCP readiness gate `agent-run boot` reaches once every precondition is satisfied (no longer boot's first act — the probe below leads and can short-circuit past it): emits the `ReadyState` JSON (or a `[PASS]`/`[BLOCKED]` line) and exits 0 iff `ready:true`, non-zero on a Blocked precondition.
- **`conductor preconditions [--json]`** — the non-mutating live-Pulse probe run BEFORE a leg is scheduled: `:4317` reachability as a connect-and-drop client, `andromeda-pulse-mcp` resolved over the inherited `PATH` without spawning, and the three `ANDROMEDA_PULSE_*` handles read presence-only. Exits 0 iff every subject is satisfied, non-zero otherwise (the documented `preflight` go/no-go exception, not the scenario-verdict rule). Prints the `[PRECONDITION]` caption — ANSI 246, one line per unmet subject naming its statement and candidate causes — and omits it entirely when satisfied; mints no `Verdict`, no `ReportState` and no per-P-ID row. It fires no canary, so it primes none of the SUT state a preflight would.
