...
- **`conductor preflight [--json]`** — the MCP readiness gate (no longer boot's first act — the probe below leads and short-circuits past it): emits the `ReadyState` JSON (or a `[PASS]`/`[BLOCKED]` line) and exits 0 iff `ready:true`, non-zero on a Blocked precondition. **`agent-run boot` does not currently reach it** — the leading probe's `handles-declared` subject is unsatisfiable, so boot short-circuits before every preflight (as measured at `conductor-0.2.0/chunks/2026-09-04-sr-findings-remediation/report.md`; the shipped path since `480bc66`). Reaching the gate today means invoking this verb directly.
- **`conductor preconditions [--json]`** — the non-mutating live-Pulse probe run BEFORE a leg is scheduled: `:4317` reachability as a connect-and-drop client, `andromeda-pulse-mcp` resolved over the inherited `PATH` without spawning, and the three `ANDROMEDA_PULSE_*` handles read for an affirmative declaration. Exits 0 iff every subject is satisfied, non-zero otherwise (the documented `preflight` go/no-go exception, not the scenario-verdict rule) — **but exit 0 is not currently reachable**: all three handles go through the same `declares()` value gate, which accepts only `"true"`/`"1"`, so the path-valued member of that SET can never declare and `handles-declared` is permanently unmet (as measured at `conductor-0.2.0/chunks/2026-09-04-sr-findings-remediation/report.md`, 2026-09-04). The `[PRECONDITION]` caption — ANSI 246, one line per unmet subject naming its statement and candidate causes — and its "omitted entirely when satisfied" arm are stated as the intended surface, not as a reachable state, until the path handle is graded by presence (route-owned). Mints no `Verdict`, no `ReportState` and no per-P-ID row. It fires no canary, so it primes none of the SUT state a preflight would.
