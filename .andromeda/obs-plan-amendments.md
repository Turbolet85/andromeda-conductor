...
**Change:** the parenthetical glossing `read_back_observed_at - journal_emitted_at` as "(Conductor's MCP round-trip)" is retired. It now reads as Conductor's journal-relative span covering the scenario's WHOLE emission window, not the MCP call, carrying its measurement pointer (`…/evidence/live-round-verdict.md`): three legs of 18s / 35s / 30s of declared phases recorded `latency_ms` 18169 / 35120 / 30212, while the sidecar's own tool calls logged `duration_ms` 0–22 in the same legs. The passage's CONCLUSION is unchanged and in fact strengthened — the budgets still bound a Conductor-side wall-clock span "rather than a Pulse-internal duration".
**Why:** **No detector proposed this** — all seven returned `proposals: []`. The obs detector surfaced it explicitly as an adjacent observation OUTSIDE its four invariants ("no detector scoped to obs-plan covers latency semantics"), and the orchestrator raised it under Validate check 5, the report substantiating the fact in its `Spec claims disproved by measurement` bullet. Playbook `:28` governs it (reconciling a spec's illustrative wording to the sound shipped implementation where the report demonstrates the invariant still holds) → routine. Sweep basis: `grep -n 'read_back_observed_at − journal_emitted_at\|read_back_observed_at - journal_emitted_at' .andromeda/*.md` → 9 hits, each READ; only this one glosses the delta as a round-trip (`architecture.md:7/:60`, `obs-plan.md:399/:408/:560` state the formula plainly, several with the correct "(wall-clock)"). A `round-trip|roundtrip` sweep across all seven masters returned hits that are all the CANARY round-trip or a serde round-trip — a different, correct usage. `grep -rn "MCP round-trip" .claude/ CLAUDE.md` → 0, so no distillation leaf carried it and none needed re-derivation for this claim. Downstream note, recorded not established: this gloss is a plausible origin of `constellation-severity-live-wiring`'s unattainable `<20s` tier — a tier chosen believing the delta ≈ a round-trip would be far too tight for a 30s emission window — which is why the residual carries both defects under one owner.
