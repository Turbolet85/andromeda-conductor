...
- **Required log fields:** `p_ids` (all 60), `missing_p_ids` (empty array on pass), `coverage_percent`, `journal_emitted_at`
...
- **Must-trace spans:** 
  ...
  - Tauri path: `tauri.command.start_scenario` (root) → `scenario.run` (same instrumentation), lines tagged with the Tauri run's `run_id` (the envelope carries `run_id`, not a `traceparent`)
