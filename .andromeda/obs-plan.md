...
| **desktop-webview** | NO auto-instrumentation — each `#[tauri::command]` handler opens a MANUAL `tracing::info_span!("tauri.command.<name>").entered()` guard, because the `#[tracing::instrument]` ATTRIBUTE does not stack with the `#[tauri::command]` macro (which rewrites the fn signature for IPC arg-extraction); measured 7/7 sites 2026-09-01, names inside §11's bounded `tauri.command.*` set | IPC envelope `run_id` correlation (no `traceparent`) |
