...
- NEVER assume the three application sites above bound every host-path channel — a MEASURED gap sits outside them (2026-09-02): the sidecar spawn (`conductor-verify/src/spawn.rs:80`, `Command::new(PULSE_MCP_PROGRAM)` with no creation flags) raises a console pane titled with the sidecar's absolute exe path that takes the OS foreground ~200 ms after `Start`; a screen reader spoke it (NVDA, SR pass row S1-01), and only the a11y leg's own scrub (`<host-path>` + a `security_finding` flag) kept it out of the committed evidence. The fix (`CREATE_NO_WINDOW`-class creation flags) is route-owned, not shipped — recorded here so the boundary's enumeration stays honest
...
- NEVER spawn an MCP sidecar via shell with operator-supplied input — fixed hard-coded program NAME resolved through the inherited `PATH` + `.env(...)` only (CVE-2026-30623 rmcp STDIO flaw, upstream §2) — and never with a visible console window: the flagless spawn publishes the resolved exe's absolute path as a foreground pane title (`spawn.rs:80`, measured 2026-09-02; `CREATE_NO_WINDOW`-class suppression is the route-owned fix, not yet shipped)
