The source brief's scenario catalog confirms: `error-baseline-spike` → P-009, P-010 (input.md line 96), and `activity-floor` → P-013, P-014 (input.md line 100). The cross-reference fix is grounded.

Now I have everything needed. The raw draft starts at `## Design Philosophy` (line 5 of the raw file; lines 1-4 are the writer's scratch notes, which are NOT part of the document). I'll re-emit the complete document starting at `## Design Philosophy`, applying exactly the four changes specified.
