Final verifications complete. Critical findings now confirmed with search evidence:

1. **`KnownResidual` and `ManualCheck`**: Both states are declared in the enum (lines 6, 113) but NEITHER has any trigger/entry-condition stated anywhere in the document. The error-handling decision (line 42) maps only three triggers ("MCP tool absent" → Blocked, "transport refused" → harness error, "wrong incident" → Fail). `degraded_mode` appears only as a `retrieve_report` tool parameter (line 125), never wired to `KnownResidual`. This is a genuine completeness gap.

2. **`run_id` format**: shown by example twice (`"2026-06-12T21-59-37-..."` line 99, dir tree `<run_id>.jsonl` line 177) but the timestamp format `YYYY-MM-DDTHH-MM-SS` is never specified as a rule. Note also `checked_at` uses colons (`2026-06-12T21:59:37Z`, line 87) while `run_id` uses hyphens — two timestamp formats, never reconciled.

3. **`blocked` row shape**: rule asserted (line 91) but envelope (lines 97-111) only shows a `Pass` row; nothing shows how `latency_ms`/`read_back_observed_at`/`verdict` populate or null when state is `blocked`.

4. **`conductor-core` membership drift**: in tree (line 167) but the Established Decisions seam list (line 43) and the crate-naming convention omit it as a named workspace member.

I have everything I need. The dimensions I'll build, ordered by impact (downstream-blocking first):

1. Cross-reference Integrity `[high]` — `conductor-core` tree-vs-prose drift, env var mapping
2. Internal Consistency `[high]` — Verdict/ReportState wall across 4 sites, version agreement, two timestamp formats
3. Contract Completeness `[high]` — blocked-row shape, `present`/`absent` domain, version-mismatch vs tool-absent paths
4. Downstream Readiness `[high]` — specialist handoff surfaces (tests/obs/security/design/a11y)
5. Decision Completeness `[high]` — `KnownResidual`/`ManualCheck` orphaned states, tier-binding rule
6. Conventions Specificity `[high]` — `latency_ms`/`slo_tier` column types, `run_id` format-by-example
7. Specialist Content Boundary `[medium]` — no logger/test-lib/frontend-framework/CSS/component-lib
8. Complexity Calibration `[medium]` — proportionality to "Personal, solo developer"

That's 8 dimensions, 6 with quoted anchor blocks (exceeds the ≥5 gates). Here is the fully rewritten meta-prompt.
