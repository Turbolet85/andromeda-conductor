I have all the files. Now I'll generate the Quiz I template combining the core fixed fields with research-derived fields grounded in the broad research findings.

The input is highly specific (Conductor — a Tauri 2 OTLP fault-injection verification harness for Pulse), so I can pre-fill several fixed fields and derive domain-specific fields from what Research I found relevant (determinism/seeding tension, MCP read-back channel, assertion policy split, emission journal/run report artifacts, OTLP transport).
