I have all four files. Let me analyze the mapping carefully.

The scope-arch Phase 0 template has these sections that need to be fillable from Quiz I + Quiz II:
1. Design Philosophy — derived
2. Stack and Technologies — Quiz II fields
3. Established Decisions — every Quiz II answer
4. Conventions — Quiz II fields
5. Standard Contracts — Quiz II fields
6. Occupied Resources — Quiz II + defaults
7. Infrastructure Patterns — Quiz II fields
8. Cross-cutting Patterns — Quiz II fields
9. Project Intent — [covered by Quiz I]
10. Inherited Defaults — derived
11. Existing Scopes — None

From research-targeted.md, the genuine forks (not pinned, not N/A) that affect the architecture document and are NOT specialist-domain are:

1. **Language/runtime** — pinned by Quiz I (Rust 2024 + tokio) → pre-filled, but tokio runtime flavor (`current_thread` vs `multi_thread`) is a genuine open fork (Trade-off #2)
2. **Framework / core structure** — Cargo workspace crate-per-seam vs single-crate (this overlaps with module boundaries — Trade-off #4) + the headless core/CLI/Tauri-bin library split
3. **Database access layer** — rusqlite vs sqlx vs SeaORM (Trade-off #3); storage model pinned, access layer is the fork; plus version bump 0.31→0.38
4. **OTLP emission strategy** — `opentelemetry-proto` raw vs `opentelemetry-otlp` SDK (Trade-off #1 — the single most consequential fork)
5. **Messaging** — N/A (single process)
6. **Deployment** — pinned (local build + run); `cargo build --release` + agent-run.sh, no real fork
7. **API style** — protocols pinned (gRPC out, MCP in, Tauri IPC internal); the open idiom is the MCP client crate (`rmcp` vs hand-rolled JSON-RPC)
8. **Validation library** — garde vs validator vs serde_valid
9. **Module boundaries** — Cargo workspace crate-per-seam vs single-crate mod (Trade-off #4, overlaps with framework)
10. **Error handling** — thiserror + anyhow vs snafu/miette (Trade-off #5)

Let me think about field ordering by impact per the constraint: language/runtime → framework → database/ORM → messaging → deployment → API style → validation library → module boundaries → error handling.

Now I'll compose the filled template.
