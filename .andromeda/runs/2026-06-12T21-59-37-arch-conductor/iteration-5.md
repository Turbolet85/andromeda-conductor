...
**Readiness gate (MCP `initialize` preflight, run once at suite start).** Asserts the negotiated protocol version and required-tool presence against the pinned contract manifest. Conceptual readiness result shape (in-memory / serialized into the run report):
...
```json
{
  ...
  "blocked_precondition": "mcp-server cargo feature + ANDROMEDA_PULSE_MCP_ENABLED",
...
- Launched child process: Pulse MCP server via `TokioChildProcess` (stdio), gated on its `mcp-server` feature.
...
- **Interface style:** OTLP/gRPC egress to `127.0.0.1:4317` (tonic 0.14.6); MCP read-back via rmcp 1.7.0 with version-pinned manifest + preflight (`blocked` on mismatch); internal Tauri 2 commands + `Channel`. No REST/GraphQL/tRPC.
...
[Iteration 1] [substantive] Named conductor-core in both authoritative member lists (Module Boundaries + Occupied Resources); reconciled run_id hyphen-stamp vs colon RFC-3339 instants; closed blocked-row NULL-field contract; defined ManualCheck/KnownResidual triggers; aggregated a Trust boundary bullet into Cross-cutting Patterns.

[Iteration 2] [substantive] Declared runs.db column types/units in the Data model conventions paragraph — latency_ms INTEGER ms (NULL for blocked), journal/read_back as integer-ms offsets, slo_tier closed TEXT enum, fingerprints JSON1 TEXT array — closing the last undeclared schema domain for the un-migratable raw-SQL index.

[Iteration 3] [substantive] Reconciled the per-run report filename in the directory tree (report.md to <run_id>.md) to honor the authoritative run_id-stem rule; the fixed literal report.md in the flat runs/ dir would be clobbered every run, collapsing the per-run Markdown ground-truth history the runs.db index queries against.

[Iteration 4] [cosmetic] reviewed, no substantive issues found — all 8 dimensions pass.
