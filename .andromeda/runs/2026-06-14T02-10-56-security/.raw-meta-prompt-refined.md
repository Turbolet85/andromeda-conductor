All anchors are now verified against fresh targeted reads. I have byte-for-byte verbatim text for every dimension's anchor, plus search evidence for the cross-reference consistency checks.

**Pass 3 (adversarial), Pass 4 (chain-of-thought protocol), Pass 5 (priority ordering)** are now synthesized. I retained 7 dimensions (within the 7-8 range), reordered by the three impact buckets, tagged each with priority, dropped nothing (every dimension has a verified anchor), and kept the Output Protocol verbatim while adding the Analysis Protocol section.
