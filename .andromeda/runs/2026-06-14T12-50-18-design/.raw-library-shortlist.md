I now have everything I need to compose the shortlist. Let me also quickly verify the developer-tool-relevant palette options that have both dark and light implications. Palette #6 Financial Dashboard, #24 Smart Home/IoT Dashboard, #81 Developer Tool/IDE, and #151 Coding Challenge are dark-native with green-on-dark. I have all the data. Let me cross-check the ANSI mapping for the top palette colors against the Color World anchors, then produce the final shortlist.

The Color World anchors map to ANSI 256 as: `#1A1B26` slate ground → ANSI 234/235 (near-black grey); `#7EE787` nominal green → ANSI 114/120; `#E3B341` hold amber → ANSI 179/221; `#F85149` fail red → ANSI 203/210; `#565F89` blocked slate-violet → ANSI 60/61; `#7DCFFF` ID cyan → ANSI 117; `#A9B1D6` journal text → ANSI 146.
