...
+----------------------------------------------------------+
|  conductor  ·  3 Pass · 1 Calib · 1 Fail · 1 Blocked      |  <- footer roll-up (Label, text-tertiary); each count token in its status color
+==========================================================+
```
...
- **Status-lamp glyph** — filled dot for a measured state, hollow ring for `Blocked` (visually distinct because never measured). `radius-full` at the icon size grid.
...
- **Verdict lamp** — in-place per-P-ID status light: `radius-full` dot at the icon size grid, color `count-nominal` (`Pass`) / `count-hold` (`CalibrationRegion`) / `status-fail` (`Fail`, held muted) / `count-blocked` (`Blocked`, hollow ring). Resolves **motionless** over a `motion-micro` color transition — never flashing (Rejected Default). Always paired with its verdict text.
...
**Distinct non-result states (never downgraded to Fail):** **Empty** → "No run yet — pick a scenario/suite to begin" (Body, `text-tertiary`); **In-progress** → "Run in progress" (Body, `text-secondary`) — the live state lives in the titlebar count + matrix, and the report never shows a partial/failed verdict for an unfinished run. Each verdict change is announced (a11y derives the live-region attribute).

### Component — Footer (status strip)

Single-line status strip pinned to the window bottom: height `space-sm`, top seam `1px` `border-subtle`, `text-tertiary`, Label role. Surfaces operational state, not navigation: `conductor` · `seed <n>` · the current run phase (`idle` / `HOLD step 14` / `run complete`) and — on the report screen — the verdict roll-up (`3 Pass · 1 Calib · 1 Fail · 1 Blocked`), each count token tinted in its own status color (`count-nominal` / `count-hold` / `status-fail` / `count-blocked`) and paired with its label so the tally is never color-only. No links, no legal, no brand chrome — a control surface, not a dashboard. Conductor never emits a native OS toast (that is Pulse behavior it observes); the footer + titlebar + matrix carry all live state in-window.
...
3. **Coverage-matrix / SLO table caption** (reinforcement) — when a run is held, the `comfy-table` summary is captioned with the frozen step-index in the hold-amber mapping, echoing where the count stopped for the agent reading the artifact.
...
preflight  protocol 2024-11-05  tools 4/4  canary ok        <- [OK] readiness gate before any scenario trusts read-back
  ...
  coverage  57 Pass · 1 Calib · 1 Fail · 1 Blocked  ·  step 60/60 done    <- summary caption; each count in its status-color map
```
...
- **cli:** linear top-to-bottom stdout, verb-noun (`conductor run|suite|report`), no cursor manipulation / no full-screen redraw (ratatui omitted). `comfy-table` 6-column coverage/SLO table with terminal-detected width; every status color paired with an ASCII bracket prefix.

**Cross-surface IA decisions:**
- Same design tokens by name across both surfaces, adapted not forked: desktop CSS vars ↔ cli ANSI codes (`count-nominal` ↔ ANSI 114, `count-hold` ↔ ANSI 179, `status-fail` ↔ ANSI 203, `count-blocked` ↔ ANSI 60, `color-id-cyan` ↔ ANSI 117, journal-text ↔ ANSI 146). The status tier maps 1:1 to typed run outcomes on both (`Pass` / `CalibrationRegion`-HOLD / `Fail` / `Blocked`). No shared URL/deep-link surface — desktop has no routes; cli has no URL coupling.
...
- The single design-system High-impact moment (the operator-pause freeze) fires only at the go/no-go hold — desktop: count freeze + 200ms dialog fade + 150ms count-tint color transition (dropped under reduced-motion preference); cli: `indicatif` stop + hold-amber HOLD line. Verdict lamps (supporting convention) resolve motionless via a `motion-micro` color transition on both surfaces — never flashing. No pulse / blink / glow anywhere; `Fail` is a motionless `status-fail` lamp, and `Blocked` is a distinct hollow-ring `count-blocked` state, never red.
