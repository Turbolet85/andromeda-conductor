This confirms the inversion clearly. The plan:
- Line 3: declares itself "the single source of truth for all test strategy + harness contracts + quality gates"
- Lines 182-186 (Section 3 "Log format"): fully defines the log format itself — structured JSONL, `tracing-subscriber` 0.3.x `format::Json`, the complete required-fields list.
- But lines 207 and 213: say the harness will "consume obs-plan §3 log format JSON schema verbatim" and "log format + envelope shape come from obs-plan §3" — making obs the producer/upstream of a format this plan already fully specifies and owns.

Per the playbook's Pass 4 note, the test plan does NOT cross-reference downstream consumers (obs/a11y/route/setup-project don't exist at test-plan generation time; tests defines the contract they later read). This is a real, downstream-blocking inversion: setup-project (most-weighted consumer) reading line 213 would block on a nonexistent `obs-plan §3` to materialize the harness, when the format is right there in Section 3. This is my strongest anchor and belongs in the Downstream Readiness dimension.

I now have all the verified material I need. Let me confirm my Pass 1 → Pass 5 synthesis and write the final meta-prompt.

**Pass 1 (Weakness Audit) findings on the current meta-prompt:**
- Dimensions 1-5, 8, 9 are mostly STRONG but several questions are long compound run-ons that bury the adversarial edge. None carry explicit anchor examples (the playbook requires per-dimension anchors + adversarial questions). That is the biggest gap: **zero anchors, zero explicit adversarial questions, no Analysis Protocol section.**
- WEAK: Dimension headings skip from 5 to 8 to 9 (mirroring a catalog numbering) — fine to keep priority tags but the refined version should number 1-N cleanly.
- MISSING: An Analysis Protocol (Pass 4 chain-of-thought + cross-reference checklist + out-of-scope discipline) — entirely absent from the current meta-prompt.
- MISSING: The downstream `obs-plan §3` inversion is not specifically targeted by dimension 5's obs bullet (which only asks whether obs can *derive* the log format — not whether the plan wrongly *depends on* obs).

**Pass 2 (anchors)** — verified above. **Pass 3 (adversarial)** — one per dimension. **Pass 4** — Analysis Protocol added. **Pass 5** — priority ordering with strongest-anchor-first within tiers.

I'll keep 7 dimensions (sharper than the input's 7), preserve all 5 mandatory dims (Scope Faithfulness, Tier Calibration, Tool Anchoring, Anti-Pattern Relevance, Downstream Readiness) and 2 conditional dims (Cross-Surface Coordination [trigger], E2E Selector Strategy [trigger]) — satisfying ≥2 conditionals. Here is the rewritten meta-prompt.
