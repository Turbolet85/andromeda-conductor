I have both files. Now I'll apply the guide to extract the a11y-relevant content.

The architecture describes a Rust OTLP fault-injection harness. Key a11y reach analysis:
- Surface is desktop (Tauri 2) + CLI/headless. Tauri = web-tech webview (axe via DevTools protocol reach); CLI/headless = manual keyboard discipline only.
- No web framework, no mobile, no testing framework like Playwright/Cypress in stack.
- The headless/CLI path is the release gate; GUI is convenience only.
