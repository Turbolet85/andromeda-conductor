I have both files. Now I'll apply the extraction guide to produce the distilled excerpt. The guide asks for four subsections: Layout Types per Surface, Error Boundary Placement, Focus Management Anchors, and Heading Hierarchy Anchors.

Let me analyze the upstream against each guide requirement:

- **Two surfaces:** desktop-webview (4 layouts/run-states) and cli (3 commands)
- **Error boundaries:** No explicit error boundary placement documented in layouts
- **Focus management:** Several signals present — modal focus trap on the go/no-go dialog, focus ring, keyboard-first navigation, focus position announced to AT
- **Heading hierarchy:** Roles named (Heading/phase line, landmark-equivalent zones), shadcn AlertDialog semantics, no explicit h1/h2/h3 numbering
