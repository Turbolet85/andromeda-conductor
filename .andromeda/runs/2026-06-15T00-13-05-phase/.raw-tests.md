The amendment history file doesn't exist, which is expected (it's created lazily on first amendment). I have all the necessary sources. Let me now extract the test requirements for this chunk.

Based on the focus guide, specialist plan, chunk scope, and absence of amendment history (empty history is normal), here is the extract:
