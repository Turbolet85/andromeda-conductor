Now I'll extract the relevant test guidance for this chunk, applying the focus guide's framework.
...
- "(tests) `cargo nextest run --workspace --profile ci` runs green (no missing `ci` profile error)." — scope def-of-done val-1 anchor.
- "(tests) Each tool exemplar passes green: rstest, proptest, insta, assert_cmd/assert_fs, cargo-llvm-cov." — scope def-of-done.
- "(tests) Determinism invariant: `ci` profile enforces `retries = 0` (no flakiness masking)." — test-plan §2 Agent-runnable invariants.
- "(tests) Machine-parseable output: cargo-nextest JUnit XML + JSONL envelope both work; `serde_json::from_str::<RunReportEnvelope>` succeeds on per-run journal lines." — test-plan §3 run command output format.
- "(tests) `cargo-audit` + `cargo-deny` green; `Cargo.lock` committed + un-drifted; `cargo clippy -D warnings` clean." — scope def-of-done, test-plan §1 supply-chain-audit.
