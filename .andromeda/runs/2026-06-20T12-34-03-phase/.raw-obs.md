Now I have the complete picture. Let me compile the obs extract for this chunk.
...
5. **Module naming conformance:** When span instrumentation arrives (Epoch 7), spans MUST use the `fault.{...}` naming convention (obs-plan §4 Fault-injection spans: `fault.silence`, `fault.ramp`, `fault.port_occupier` are the current bounded set; `fault.{...}_train` will be the pattern for bursty-train, if added).
...
2. **Duty-cycle accessor semantics:** Expose `active_duration_ms()`, `quiet_duration_ms()`, and within-cycle phase query (e.g., `is_active_at(offset_ms: u64) -> bool`) matching the timeline/`PhaseSpec` boundary consumption (obs-plan §1 Instrumentability table: "the `*_ms` accessors the timeline/`PhaseSpec` boundary will consume").
...
3. **NEVER introduce OTel SDK, tracing init, or logger config:** The primitive itself is unobserved; the timeline/scheduler (Epoch 7) will wrap its use with spans (obs-plan §11 Telemetry Strategy anti-pattern: "NEVER introduce an OTel SDK + exporter").
...
1. **(obs-deferred) Fault instrumentation:** When timeline wiring occurs (Epoch 7), span `fault.{...}_train` (or equivalent bursty-pattern span name per obs-plan §4 bounded set) wraps pattern application; span attributes include `fault_type` ("bursty_train"), `fault_duration_ms` (per active/quiet window), `fault_start_offset_ms` (journal offset).
2. **(obs-now) Zero-unlogged-panics:** `BurstyTrain::new()` returns `Result<Self, FaultError>` on invalid duty cycles; zero panics in bounds validation (obs-plan §10).
3. **(obs-later) Run-report envelope fields:** Any scenario-specific field added to the JSONL envelope (if bursty-train scenarios require cycle-phase markers) must pass redaction layer and field-allowlist checks (obs-plan §11 PII Scrubbing: no absolute paths, internal struct names dropped by allowlist).

## Relevant amendment history
1. **2026-06-18-severity-logs** — `emit.logs_batch` added to bounded span-name set alongside `emit.batch` (distinguishing logs egress from trace egress). Relevant as precedent for bounded-set augmentation: when bursty-train fault instrumentation ships (Epoch 7), a new span name `fault.{...}_train` will follow the same conforming-member pattern (spec → sound-impl → bounded-set add, no high-cardinality names).