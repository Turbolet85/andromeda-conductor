Now I have all the required information. Let me analyze the chunk scope and extract the relevant security requirements.

The chunk is for `BurstyTrain` (P-013), a deterministic fault-helper primitive in `conductor-faults` that describes the bursty-train activity pattern (active/quiet cycle).
