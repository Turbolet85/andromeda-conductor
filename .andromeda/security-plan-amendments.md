...
**Change:** The Universal bullet keeps the probe's non-mutating / no-spawn / no-env-write properties (unaffected and restated) but WITHDRAWS the "that is not a downgrade" reading: because `declares()` value-gates all three handles, the PATH-valued `ANDROMEDA_PULSE_DATA_DIR` never satisfies `handles-declared`, the probe's exit is unconditionally non-zero, and `boot` has skipped the MCP readiness gate on every run since `480bc66` — which IS a downgrade of that path until the handle is presence-graded. The §Input Validation row now separates the two declarable BOOLEAN handles from the PATH handle, keeps "nothing to validate" (no value becomes a path, argv element or log value), and records that a present-and-correct data-dir is presently indistinguishable from an absent one.
**Why:** Report §Spec claims disproved by measurement #1. Routine under playbook.md:118 (a measured product defect with zero delta at its site, dispositioned by an operator directive that names the defect itself); applied as a record, with the fix route-owned. **Search performed:** grep of security-plan.md for `preconditions`, `declares`, `ANDROMEDA_PULSE_`, `downgrade`, `short-circuit`, `presence-only` — two sites carried the claim; both amended.
...
**Change:** Retires the entry directly above. The Universal bullet's "not a downgrade" reading HOLDS again: the probe grades the PATH handle by presence via `conductor_core::handle_declared` (the two flag handles keeping the truthy gate through the value-only `flag_declared`), so `handles-declared` is satisfiable, the short-circuit fires only on a genuinely unmet subject, and `boot` reaches the MCP readiness gate. The ban's SCOPING is kept unchanged (it binds a preflight that IS invoked), as are the probe's non-mutating / no-spawn / no-env-write properties. The READ SET row is re-pointed from `declares()` value-grading to `conductor_core::{flag_declared, handle_declared}` and records the non-widening witness; "nothing to validate" still holds. The spawn-propagation row's parenthetical is corrected to say the declaration reader grades this handle on PRESENCE — **the spawn-side duty itself is untouched** (`.env(...)`-only, metacharacter rejection, never in argv).
**Why:** Report §Outcome — `handles-declared` satisfiable (probe exit 0, 2026-09-04T17:07Z); non-widening witness MET (`ANDROMEDA_PULSE_DATA_DIR=false` declares, `ANDROMEDA_PULSE_MCP_ENABLED=false` does not, because the run-contract path routes only to the value-only `flag_declared` and structurally cannot reach the presence arm); handle-NAME-only MET (a distinctive data-dir value appeared 0 times across the probe, `--json` and both live `boot` artifacts; the gate's `data_dir` renders `<redacted>`). **Operator-ratified at this wrap (escalation E1):** D-security-input is escalate-severity and no playbook rule covered "the master's own text names this chunk as the fix's owner and the chunk shipped it"; the operator ruled apply-all-three and approved minting that rule. Rule `:124` (boundary widening, never-routine) was checked and does NOT match — the probe grants nothing and passes nothing onward, the blocking gate is behaviourally unchanged, and this plan's own withdrawn bullet identified the PRE-chunk state as the downgrade. **Search performed:** grep of security-plan.md for `permanently unmet`, `route-owned`, `not shipped`, `WITHDRAWN`, `downgrade`, `true"`/`"1"` and `presence`, plus `declares` / `boot` / `preconditions` read for the mechanism however worded — three sites carried it (`:114`, `:120`, `:367`); all three amended; post-edit re-grep returns 0 for every retired phrase.
