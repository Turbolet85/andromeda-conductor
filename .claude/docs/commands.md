...
- `agent-run.sh run [--unit|--integration|--e2e|--live [real-model]]` — nextest + doctest + clippy + scenario runs. `--live` is the operator-gated live-Pulse suite (needs a running `pulse-app`; never a CI gate, never reachable from a bare `run`) — it refuses at exit 1 naming each unmet precondition rather than firing a leg. `--live real-model` runs the ONE real-model leg instead (`real-model-interpretation`, led by `conductor preconditions --for real-model-interpretation`; needs `pulse-app` under the real-model posture — deterministic L4 absent or falsy); an unknown selector exits 2. Scenario leg: `SCENARIO=<name|P-ID> [SEED=<n>] agent-run.sh run` — `--seed` is appended ONLY when `SEED` is explicitly set (otherwise the TOML-declared seed governs); a P-ID picks the first scenario naming it in directory order, so name the scenario where several share one.
...
- `conductor run <scenario|P-ID> --seed <s>` — drive one scenario, its gate evaluated under the scenario's declared L4 posture (a P-ID named by several scenarios resolves to the first in directory order — name the scenario).
- `conductor preconditions [--for <SCENARIO>] [--json]` — the non-mutating live-Pulse probe; `--for` grades the handles under that scenario's declared L4 posture (a P-ID is refused, exit 2).
...
- Webview E2E: `agent-run.{sh,ps1} run --e2e` → ensure-frontend → `cargo build --release -p conductor-tauri --features tauri/custom-protocol` (the FEATURE, not the profile, embeds the bundle) → seed the fixture runs dir (the committed `crates/conductor-run/tests/fixtures/lamps-journal.jsonl` copied into the gitignored `runs/e2e-fixture/`, with the tauri-driver spawn carrying `CONDUCTOR_RUNS_DIR=runs/e2e-fixture` — `/runs/` is gitignored, so without a seed the coverage rows have no run record to render) → `wdio run` (tauri-driver). Headless under `xvfb` on a Linux host (the TARGET CI arrangement; `ci.yml` runs two `windows-latest` jobs and no webview-E2E job, measured 2026-09-06); headful on the Windows host when `CONDUCTOR_MSEDGEDRIVER` names a driver — unset or not-a-file ⇒ the leg skips at exit 0 with the fetch recipe. macOS has no WebDriver. Operator-local siblings over the same stack (never `agent-run`, never CI): `npm run a11y:driven` (the live-Pulse hold dialog) and the screen-reader leg `npm run a11y:sr-empty` / `a11y:sr-error` / `a11y:sr` (NVDA's speech log under `CONDUCTOR_NVDA`, unset ⇒ skip at exit 0; the `sr` subject needs the live-Pulse firing form and fires one canary) — all from `crates/conductor-tauri/ui`. **The `--e2e` arm asserts the PRINTED verdict, not its exit code** (both shells, identical semantics, since 2026-09-07 — a property of the scripts TOGETHER with the invoking environment: a caller setting `$PSNativeCommandUseErrorActionPreference = $true` preempts the capture-then-print and the verdict never prints, measured 2026-09-07): the exit alone cannot separate a full pass from a total skip, so it requires zero on the `Spec Files:` failed count, a skip tally within the expected-skip SET (a third skip is a failure), and the `[webview2 … windows]` driven-session banner — and it emits one violation record per run at `runs/a11y/<run_id>.jsonl`. Setting `CONDUCTOR_A11Y_STRICT=1` (the CI posture) makes an unresolved `CONDUCTOR_MSEDGEDRIVER` / `CONDUCTOR_NVDA` exit NON-ZERO instead of skipping.
