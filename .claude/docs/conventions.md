...
- **The garde sibling boundary:** garde 0.22.1's field-level `custom` receives `(&field, &())` and can see no SIBLING field. An invariant contained WITHIN one field can lift one altitude up as a `custom` on that field (`fault_phases_are_silent` on `phases`); an invariant spanning a field and its sibling ships instead as a plain load-path `Scenario::check_*()` invoked from `from_toml_str` (`check_capabilities`, `check_budgets`).
