...
- Directory layout: `crates/` (**9** workspace members — 7 seam/library crates + the `conductor-cli` and `conductor-tauri` bins) · `scenarios/` (config files, each naming its P-IDs — a P-ID may be named by several) · `contracts/` (pinned MCP manifest + SUT capability manifest + SUT load envelope + SUT run contract + the scenario-assertion audit ledger + the P-025 measurement contract + the real-model leg posture — the two members no Rust code reads) · `runs/` (journals + reports + `runs.db`) · `scripts/` (`agent-run.{sh,ps1}`). See architecture.md §Infrastructure Patterns for the full tree.
