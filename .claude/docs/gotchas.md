...
**Fix if broken:** Clean the checkout (or bypass it — `cargo audit --db <empty dir>` clones fresh), then re-run; a residue red disappears with no dependency change. Post-clean this workspace measures exit 0 (1239 advisories, 564 packages, 18 `deny.toml`-adjudicated allowed warnings).
