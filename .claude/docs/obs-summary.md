...
- **Log format:** JSONL, one object per line; required fields `journal_emitted_at`, `run_id`, `seed`, `scenario`, `p_ids`, `verdict`, `state`, `latency_ms`, `slo_tier`, `fingerprints` (schema owned by test-plan §3).
