...
- CLI args + the RUST-READ `CONDUCTOR_*` env path handles → `std::fs::canonicalize` + bounds-check at the CLI edge (outside garde). `CONDUCTOR_MSEDGEDRIVER` and `CONDUCTOR_NVDA` are harness-only (read by `wdio.conf.ts`, never by a Conductor binary) → existence + `isFile` + shell-metacharacter rejection at the wdio edge, then an array-form spawn (the driver path as an argv element, the NVDA path as the program of a fixed-argv detached spawn). The canonicalize duty binds per READER, not per handle (2026-09-06): `CONDUCTOR_RUNS_DIR` also has TEST-binary readers — `conductor-run/tests/journal_conformance.rs` and (recorded 2026-09-23) the two operator-gated `live-pulse` captures `live_suite.rs` / `real_model_live.rs` — which apply none, carrying recorded residuals (the captures' hardening is route-owned). `CONDUCTOR_E2E_SEED_DIR` is a third class (2026-09-02) — harness-SET, TEST-read, never read by a shipped binary; a repo-relative constant carried in the spawn's env map, guarded by neither rule, a no-op when unset. The `sr*` leg's speech-log ingest is a boundary of its own: untrusted third-party text, bounded and host-path-scrubbed before the evidence record is committed. So is the real-model capture ingest (2026-09-22): untrusted SUT output — an MCP re-read plus Pulse's own log, the latter read through `ANDROMEDA_PULSE_DATA_DIR`'s value as a path with no canonicalize (a recorded residual) — passed through `redact_value` + `mask_host_paths`, fields-only, fingerprints elided, before the committed `evidence/rm-capture.txt`. The sidecar spawn suppresses the child's console window on Windows (`CREATE_NO_WINDOW` under `#[cfg(windows)]`, shipped 2026-09-04), closing the measured host-path disclosure the pane title carried.
