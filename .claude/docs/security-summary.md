...
- A third dependency class (2026-09-08) sits outside every lockfile gate: the CI-time-fetched WebView2 Evergreen bootstrapper, admitted by pre-execution Authenticode verification alone. Version posture since 2026-09-12 is FLOOR-CONDITIONAL — fetched only when the measured pre-install runtime major is below 152, with the `≥ 152` floor assertion outside the conditional; the always-latest rationale was exhausted when the probe's subject moved from the runtime major to the driver/runtime skew. Pinned once the job gates — still unmet.
