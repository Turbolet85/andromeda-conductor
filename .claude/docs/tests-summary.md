...
- **Per-surface drivers:** assert_cmd (cli release gate) · the hand-rolled JSON-RPC `stub_pulse_mcp` (MCP, CI — rmcp removed 2026-06-27; the stub's incident item key is `incident_id`, the live sidecar's) · tauri::test mock runtime (IPC) · tauri-driver/WebdriverIO (webview — Linux+xvfb in CI, headful on the Windows WebView2 host under `CONDUCTOR_MSEDGEDRIVER`; macOS driver-less). A live-Pulse leg is operator/local only and may ride a cargo-feature gate (`--features live-pulse`), which owes its own clippy leg.
