...
- **Mutation instrument:** cargo-mutants — operator/local audit, never a CI stage; full-crate-path `-f`, `--test-tool=nextest` required, `Found 0 mutants` is a NO-OP not a pass. Named survivors end killed or accepted-deliberate against a cited rule (test-plan §4/§9/§10).
