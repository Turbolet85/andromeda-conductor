...
- **Mutation instrument:** cargo-mutants — operator/local audit, never a CI stage; full-crate-path `-f`, `--test-tool=nextest` required; the exit code carries no verdict either way (`Found 0 mutants` is a NO-OP at exit 0; a non-zero exit reflects surviving/timeout classes) — gate on the tallies (`missed.txt` == exactly the run's accepted-deliberate survivors, empty only when none is accepted; every other named survivor in `caught.txt`). Named survivors end killed or accepted-deliberate on either of two bases — a standing rule prescribes the untested shape, or the mutant is proven EQUIVALENT (scoped to that mutant, never its comparison class). A unit is REGISTERED by its `[[unit]]` declaration, never by its row count (2026-09-14): an all-killed declared unit passes on an empty `missed.txt`, the declaration may carry a `files` scope applied via `-f`, and a 0-mutant run is an explicit FAIL; the gate reads `missed.txt`/`caught.txt` only and never `timeout.txt`. The accepted roster has TWO joined forms — test-plan §12 is the citation home (rule + evidence per member) and `scripts/mutation-roster.toml` the coordinate-free executable form the committed gate `python -X utf8 scripts/mutation-gate.py {unit}` compares a run's `missed.txt` against as a MULTISET over `(file, mutation)` — joined on a `member` key; a member is identified by its mutation DESCRIPTION, never by `file:line:col` (retired 2026-09-13). Coverage exclusion is the shipped generic regex `--ignore-filename-regex '[\\/]tests[\\/]'`, which does NOT exclude the src-side `stub_pulse_mcp` (test-plan §4/§9/§10/§12).
