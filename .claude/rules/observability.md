...
- **Sinks:** cli → stderr (dev) / `logs/agent-latest.jsonl` (`--agent-mode`); Tauri backend → `conductor-tauri.jsonl` under `runs_dir.parent()/logs/` (the directory rides `CONDUCTOR_RUNS_DIR`: the project-root `logs/` only with it unset; `runs/logs/` · `runs/driven/logs/` · `runs/sr-leg/logs/` on the a11y suites, measured 2026-09-02); Tauri frontend → browser `console.log` JSON only (NO browser OTLP exporter — recursion guard).
