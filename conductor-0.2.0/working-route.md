...
[2026-09-08-hosted-runner-webview2-session] Hosted-runner WebView2 session — the a11y job's first green run (v2-24 claimed or deferred)
   ↓
WebView2 runtime 152+ installed in-job — the one variable every passing case shares and the failing case lacks (v2-24 claimed or deferred)   PREREQ: close rust gate deferral (deferred since `2026-09-08-hosted-runner-webview2-session`)   CARRY (from `2026-09-08-hosted-runner-webview2-session`, minted here by operator wrap-directive item 2): **the hosted `windows-2025` image's WebView2 runtime opens no remote-debugging endpoint, and this is measured DRIVER-INDEPENDENT** — probe (a) is a bare launch with no driver in it at all: `WEBVIEW2_USER_DATA_FOLDER` is honoured (`EBWebView` created under it), the app stays alive the whole window, and `DevToolsActivePort` never appears within 90 s, where the identical form on the dev host produces it in 1 s (run 34234558853; `chunks/2026-09-08-hosted-runner-webview2-session/evidence/probe-verdict.md`). Everything downstream — msedgedriver's 60 s wait, `session not created`, tauri-driver's relay — follows from that, so **the driver was never the discriminator**. **The version axis is the one that survives, on the RUNTIME not the driver:** passing = {runtime 152.0.4191.x × driver 151, 2026-09-02 morning} ∪ {runtime 152.0.4191.x × driver 152.0.4191.53, since 2026-09-02 14:45}; failing = {runtime 151.0.4129.101 × driver 151.0.4129.101} on the runner. Every passing case runs runtime **152**; the single failing case runs **151**. **The probe:** install WebView2 runtime 152+ in-job from the signed Evergreen bootstrapper, with `Get-AuthenticodeSignature` CHECKED before it runs, then re-run probe (a) — ONE push. The `a11y` job stays RED on the build branch meanwhile, and **no gate step gains `continue-on-error`**. **Retired on evidence, do not revisit:** H1 (official `tauri-driver` crate) and H5 (`tauri-plugin-automation` + test-runner-backend) are both driver-layer changes and the fault is upstream of every driver; H2 (`WEBVIEW2_USER_DATA_FOLDER`) is doubly out — the driver overrides an inherited value AND probe (a) shows the handle honoured with the port file still absent; H3 as driver-skew is rejected (runner driver = runtime = Edge, all 151, self-consistent). **H4** (self-hosted Windows runner) is route-forward ONLY — none exists for this project (operator-confirmed 2026-09-08) and the repo is public. **`v2-24` remains `planned` and pooled** — a `deferred` status is terminal until the next version's intake, so it waits for this probe; its `notes` already carry measurements (a)/(b)/(c) and the runtime hypothesis, and **this entry owns claiming or deferring it**. `hypothesis:` probe (b)'s `count: 0` (no fresh `scoped_dir` seen) is NOT a conclusion — the SR chunk's runner verbose log shows the driver populating `Preferences`, so a profile dir existed there; candidates are a `TMP` vs `TEMP` divergence in the step shell, or removal inside the 1 s poll interval. Unmeasured.
   ↓
