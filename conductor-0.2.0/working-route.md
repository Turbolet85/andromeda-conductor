...
Per-check latency measurement — sub-5s budgets beneath the closed slo_tier set, per-check latency_ms carried in the run-report envelope
   ↓
Delegated timing budgets proven — halo hue, constellation discovery, report render and counter refresh at real values (P-025, P-027, P-037, P-045)   BLOCKED-ON: three of the four capabilities have no timing observable on EITHER side. Only P-037 has one (`metric.report.render_ms`, `pulse-app/src/incidents_router.rs:561`); P-025 / P-027 / P-045 have no `metric.*` target in Pulse's 18-target inventory and no latency assertion harness-side either (`halo-hue-encoding.toml:27` grades the hue by operator observation; `findings-counter-refresh.toml:10-11` calls its <=1s timing an unbuilt measurement with no content token). Asserting the other three requires Pulse to BUILD three observables, each a TauRPC quadruple binding plus an allowlist leaf plus an a11y-gated webview change — Pulse-side work, not Conductor's. Evidence: Pulse matrix P-075 `notes`, blocker 1 (2026-08-17 visit). Clear this annotation when those observables ship.
   ↓
