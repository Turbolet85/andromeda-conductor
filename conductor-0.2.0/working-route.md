...
Run-report envelope conformance gate — every run journal row schema-complete and host-path-free, build failing on violation
   ↓
Coverage completeness gate — zero-gap classification over the current SUT set with every CI gate green   CARRY (from 2026-08-09-current-sut-coverage-classification): **decide whether `coverage-matrix.md` becomes a real committed artifact.** `architecture.md` §Occupied Resources registers it as the definition-of-done artifact and `test-plan.md` §1 Critical Path 6 names it as the gate's surface, but the file has NEVER existed in this repo — `conductor coverage --write` generates it on demand and that chunk deleted the one it produced while verifying, since a plan declaring zero new files would otherwise have had `git add -A` commit it. This entry owns the call (commit it + gate on it · keep it on-demand and re-word the two specs · or gitignore it); the count/mode staleness on that same test-plan line was already amended, only the artifact question is open
   ↓
