...
[2026-09-07-sr-findings-fixed] SR findings fixed — re-announce after first focus (1), sequential-focus start point measured then fixed (2), announcement variance recorded (3)
   ↓
Hosted-runner WebView2 session — the a11y job's first green run (v2-24 claimed or deferred)   CARRY (from `2026-09-07-sr-findings-fixed`, minted here by operator wrap-directive item 1): **on the hosted `windows-2025` runner a WebView2-mode session never exposes `DevToolsActivePort`** — through `@crabnebula/tauri-driver` and direct at `msedgedriver` alike, as measured at run 34162118841. The a11y job is correctly wired and correctly RED; two defects were fixed at that chunk and are proven in CI (the driver handle now resolves — `${{ env.EDGEWEBDRIVER }}` expands empty because GitHub's expression context holds no runner-process variable, so it is resolved in the step shell; and wdio's RED output now reaches the job log, which the step's `$PSNativeCommandUseErrorActionPreference` had been destroying). Neither makes it green. **Excluded by measurement, all of them:** runtime absence (WebView2 Evergreen Runtime present) · version mismatch (msedgedriver = Runtime = Edge, all 151.0.4129.101, self-consistent; this dev host runs 152) · app crash (launched alone on the runner the app stays up with three `msedgewebview2.exe` children in Session 2, zero-byte stderr, no Crashpad dumps) · GPU/sandbox · UDF/path (WebView2 mode passes no `--user-data-dir` at all and the verbose log names no path). **The cause beyond that statement is UNMEASURED** — do not inherit a mechanism for it. **RETRACTED, do not rebuild on it:** an earlier reading that msedgedriver "launched the Tauri binary as though it were Edge" was a PROBE ARTIFACT — the isolation POST omitted `browserName: "webview2"`, without which the same POST fails on the DEV HOST too against a binary that passes 12/12 through tauri-driver; with it the driver's side is provably correct on the runner (env var populated, app launched bare) and the session still fails. Validate any diagnostic form where the real path PASSES before building on its failure. **Tooling already in place:** the two `continue-on-error` diagnostic `ci.yml` steps (driver+runtime versions; session isolation, `if: always()`, with the `a11y-session-diag` upload) and `conductor-0.2.0/chunks/2026-09-07-sr-findings-fixed/evidence/ci-msedgedriver-verbose.log` (the corrected webview2-mode run, path-scrubbed). **Next probes, every one `hypothesis:` and none measured:** `hypothesis:` the official `tauri-driver` crate vs `@crabnebula/tauri-driver`'s translation · `hypothesis:` an explicit `WEBVIEW2_USER_DATA_FOLDER` under `$RUNNER_TEMP` · `hypothesis:` driver 151's WebView2 mode vs this host's 152 · `hypothesis:` a self-hosted Windows runner as the fallback. **v2-24 is UN-CLAIMED and pooled** (`chunk: null`, status `planned`) with this premise and its retraction in its `notes`; this entry owns claiming or deferring it. **The *A11y CI gate* entry's `BLOCKED-ON` clearing event — one GREEN CI run of the a11y job after a push — is NOT met**; its premise lives here now, and is deliberately NOT re-raised as a fresh `BLOCKED-ON` on *Release build and bundle*.
   ↓
