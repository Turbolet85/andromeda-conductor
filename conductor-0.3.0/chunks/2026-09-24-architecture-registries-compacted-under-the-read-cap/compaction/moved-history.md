# Moved history — 2026-09-24-architecture-registries-compacted-under-the-read-cap

_Passages that left `architecture.md` §Established Decisions / §Occupied Resources when the two registries were compacted under the read cap, verbatim, for wrap P2 to append to `architecture-amendments.md`. Each is either history or the BEFORE wording of a sentence the body now states more briefly; history the sidecar already carries is not repeated here (its ledger row names the entry that holds it)._
