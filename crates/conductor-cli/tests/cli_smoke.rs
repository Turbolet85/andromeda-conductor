...
fn copy_load_envelope(dir: &TempDir) {
    let src = format!("{}/../../contracts/pulse-load-envelope.toml", env!("CARGO_MANIFEST_DIR"));
    let toml =
        std::fs::read_to_string(&src).unwrap_or_else(|e| panic!("read pulse-load-envelope.toml: {e}"));
    dir.child("contracts/pulse-load-envelope.toml").write_str(&toml).unwrap();
}
