...
mod tests {
    ...
    async fn path7_the_two_surfaces_write_an_equal_envelope_into_one_runs_db() {
        // Cross-surface parity (test-plan §5/§6 Path 7, verification-matrix v2-25). The two arms are
        // the mandated pair — the `tauri::test` crate driving the SAME conductor_run composition
        // start_run's thread runs, and the `assert_cmd` CLI subprocess — pointed at ONE runs dir so a
        // single runs.db holds both rows.
        //
        ...
        assert_cmd::Command::cargo_bin("conductor")
            .expect("the conductor binary builds")
            ...
            .assert()
            .success();
        let cli_run_id = conductor_core::latest_run_id(&runs_dir)
        ...
        // SAFETY: this is the only set_var in this binary and it is set before any runtime or thread
        // reads env; the CLI arm passes the same value through the child's own .env instead.
