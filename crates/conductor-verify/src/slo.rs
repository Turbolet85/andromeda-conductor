...
pub struct SloOutcome {
    ...
    pub latency_ms: i64,
    /// Whether the latency met the tier deadline (`0 ≤ latency_ms ≤ tier.deadline_ms()`).
    pub within_tolerance: bool,
}

/// Compute the journal-relative SLO outcome against a tier's deadline.
///
/// Latency is `read_back_observed_at_ms − journal_emitted_at_ms`; a negative latency (read-back
/// observed before emission — a clock anomaly) is never within tolerance.
pub fn evaluate_slo(
    tier: SloTier,
    ...
    let within_tolerance = (0..=tier.deadline_ms()).contains(&latency_ms);
    SloOutcome { latency_ms, within_tolerance }
}
...
///
/// A `Hard` claim matches only when BOTH the comparison holds AND the latency is within the tier
/// deadline. A sample-count floor ([`ComparisonKind::CountAtLeast`]) that is unmet routes to the
/// calibration region regardless of the declared class — sample floors never hard-fail (architecture
/// §Timing-Tolerance Model). Infallible; `observed`/`expected` are redacted inside [`classify`].
pub fn evaluate_check(
    ...
    let slo = evaluate_slo(tier, journal_emitted_at_ms, read_back_observed_at_ms);
...
mod tests {
    ...
    fn slo_within_and_exceeded_per_tier() {
        assert!(evaluate_slo(SloTier::Tier5s, 1000, 5000).within_tolerance); // 4000 ≤ 5000
        assert!(!evaluate_slo(SloTier::Tier5s, 1000, 6001).within_tolerance); // 5001 > 5000
        assert!(evaluate_slo(SloTier::Tier5s, 0, 5000).within_tolerance); // boundary
        assert!(!evaluate_slo(SloTier::Tier5s, 0, 5001).within_tolerance);
    }

    #[test]
    fn negative_latency_is_never_within_tolerance() {
        let o = evaluate_slo(SloTier::Tier90s, 5000, 1000);
