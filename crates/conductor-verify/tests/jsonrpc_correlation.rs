...
async fn request_ids_advance_so_a_response_pairs_to_exactly_one_request() {
    ...
    let config = StubConfig { wire_log: Some(log.clone()), ..StubConfig::default() };
    ...
    assert_eq!(ids.len(), 3, "initialize + tools/list + one tools/call reached the wire: {ids:?}");
...
async fn a_stale_response_is_skipped_rather_than_paired_to_a_later_request() {
    ...
    let config =
        StubConfig { decoy_before_nth_request: Some(2), ..StubConfig::default() };
    ...
    assert_eq!(tools, expected, "the real response is the one that answers: {tools:?}");
}
...
async fn the_initialized_notification_reaches_the_wire_after_initialize() {
    ...
    let initialized = entries
        .iter()
        .position(|entry| {
            matches!(entry, WireEntry::Request { method, .. } if method == "initialize")
        })
