...
def expected_for(unit: str) -> tuple[Counter[tuple[str, str]], list[dict]]:
    roster = tomllib.loads(ROSTER.read_text(encoding="utf-8"))
    rows = [e for e in roster.get("entry", []) if e["unit"] == unit]
    return Counter((r["file"], r["mutation"]) for r in rows), rows


def complete(out: Path) -> tuple[bool, str]:
    ...
    if not outcomes.is_file():
        return False, f"no outcomes.json under {out}"
    data = json.loads(outcomes.read_text(encoding="utf-8"))
    if not data.get("end_time"):
        return False, "outcomes.json end_time unset — the invocation is still running"
    total = data.get("total_mutants")
    if mutants.is_file():
        planned = len(json.loads(mutants.read_text(encoding="utf-8")))
        if total != planned:
            return False, f"total_mutants {total} != len(mutants.json) {planned} — partial run"
    else:
        return True, f"end_time set; mutants.json absent, total_mutants {total} (disclosed)"
    return True, f"end_time set; total_mutants {total} == len(mutants.json)"
...
def main() -> int:
    if len(sys.argv) != 2:
        ...
        return 2
    unit = sys.argv[1]
    expected, rows = expected_for(unit)
    if not rows:
        print(f"MUTATION GATE {unit}: FAIL — no roster rows for this unit")
        return 1
    ...
    proc = subprocess.run(
        ["cargo", "mutants", "-p", unit, "--test-tool=nextest", "--jobs", "2",
         "--output", str(output)],
        cwd=ROOT,
    )
